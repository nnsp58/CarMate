...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Login' button on the welcome page to open the login screen (use interactive element index 100).
        ...
        # -> Type the provided email and password into the respective fields and click the 'Login' button to authenticate (use inputs 126 and 127, click 141).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the Login button to submit credentials and trigger the post-login redirect (use interactive element index 205). Then wait for the page to change and check for '/home'.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/flutter-view/flt-semantics-host/flt-semantics/flt-semantics/flt-semantics/flt-semantics[2]/form/flt-semantics[6]').nth(0)
        await asyncio.sleep(3); await elem.click()
        ...
        assert await frame.locator("xpath=//*[contains(., 'Passengers')]").nth(0).is_visible(), "Expected 'Passengers' to be visible"
